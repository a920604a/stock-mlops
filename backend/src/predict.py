...
class Predictor:
    def __init__(self, ticker: str, exchange: str):
        ...
        logger.info(f"✅ 使用模型: ticker={self.model_meta.ticker}, features={self.model_meta.features}, model_type={self.model_meta.model_type}")
    ...
    def predict_next_close(self, target_date) -> Optional[float]:
        ...
        df = load_stock_data(self.ticker, exchange="US", start_date=fetch_start, end_date=fetch_end)
        ...
        if not actual_df.empty:
            ...
            logger.info(f"🔍 預測 {self.ticker} {target_date.date()} 收盤價為：{predicted_price:.2f}，實際收盤價為：{actual_close:.2f}")
        else:
    ...
    def log_prediction(self, predicted_price: float, target_date: datetime):
        ...
        logger.info(f"✅ 已記錄預測：{self.ticker} {target_date.date()} 的收盤價 = {predicted_price:.2f}")
