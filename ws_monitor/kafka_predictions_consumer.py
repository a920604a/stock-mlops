...
def calculate_baseline_metrics(reference_data):
    snapshot = drift_report.run(reference_data=reference_data, current_data=reference_data)
...
def check_anomalies(drift_result):
    ...
    current_missing_ratio = extract_metric_value(drift_result, "DatasetMissingValueCount")
    ...
    current_missing_count = extract_metric_value(drift_result, "MissingValueCount", "actual_close")
    if current_missing_count and current_missing_count > MISSING_COUNT_THRESHOLD:
        alerts.append(f"Missing count for actual_close too high: {current_missing_count}")
    ...
    if current_mean is not None and baseline_mean is not None and abs(current_mean - baseline_mean) > MEAN_DIFF_THRESHOLD:
        alerts.append(f"Mean value drift detected: baseline={baseline_mean:.2f}, current={current_mean:.2f}")
    ...
    if current_std is not None and baseline_std is not None and abs(current_std - baseline_std) > STD_DIFF_THRESHOLD:
        alerts.append(f"Std deviation drift detected: baseline={baseline_std:.2f}, current={current_std:.2f}")
...
async def kafka_predictions_consumer_loop():
    ...
    while True:
        try:
            ...
            logger.warning(f"[KafkaConsumer][predictions] Kafka not ready, retry in 2s: {e}")
        ...
        async for msg in consumer:
            try:
                ...
                if reference_data is None and len(current_data) >= BASELINE_SIZE:
                    ...
                    logger.info(f"[KafkaConsumer][Evidently] Baseline metrics: {baseline_metrics_dict}")
                    
                # Step 2-2: 執行 drift 檢查及異常判斷
                elif reference_data is not None and len(current_data) >= DRIFT_CHECK_SIZE:
                    try:
                        snapshot = drift_report.run(reference_data=reference_data, current_data=current_data)
                        drift_result = snapshot.dict()
                        logger.info(f"[KafkaConsumer][predictions][Evidently] Drift check result: {drift_result}")
                        ...
                        logger.warning(f"[KafkaConsumer][predictions][Evidently] Drift check failed: {e}")

                # Step 3: 廣播 Prediction 給 WebSocket
                await broadcast_predictions(data)

            except Exception as e:
                logger.error(f"[KafkaConsumer][predictions] Error processing message: {e}")
